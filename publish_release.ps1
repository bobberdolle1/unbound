...
$releaseNotes = @"
# Unbound v$VERSION - Critical Hotfix
...
if ($gitStatus) {
    git add .
    git commit -m "chore: release v$VERSION - Critical Hotfix"
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "Release v$VERSION exists. Deleting..." -ForegroundColor Yellow
    gh release delete "v$VERSION" --yes
}

gh release create "v$VERSION" "$INSTALLER_PATH" --title "Unbound v$VERSION - Critical Hotfix" --notes-file $RELEASE_NOTES_FILE --latest
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "================================================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "Release: https://github.com/bobberdolle1/unbound/releases/tag/v$VERSION" -ForegroundColor Cyan
