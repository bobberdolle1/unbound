...
Production V1 builds a closed three-candidate catalog per normalized target hostname and reports `READY` only after every candidate canonicalizes and fingerprints successfully. All candidates are explicit-host, outbound TCP/TLS port 443 strategies: byte-1 multisplit (`LOW`), TLS overlap multisplit using trusted logical `tls-google` (`MEDIUM`), and hostfakesplit using the reviewed static `ozon.ru` template (`MEDIUM`). The latter retains its TCP timestamp preflight requirement. The catalog neither derives from legacy profile names nor includes the developer-only acceptance strategy or high-risk fake-TLS fixture. Product inputs accept HTTPS only. The internal target retains its path and query for the Observatory, while the public result omits userinfo, query, fragment, and secrets. Candidate `planner_status` is the Planner candidate status; the result-level `planner_disposition` remains the Planner report disposition.

### Experimental UI flow

`AutoTune vNext (эксперимент)` is a secondary explicit action beside the legacy primary AutoTune action. It resolves `youtube-web`, `discord-api`, and `steam-store` from a typed product registry that reuses the diagnostic HTTPS endpoints; it always supplies the product-owned Cloudflare trace control. A custom target is accepted only as validated HTTPS, and result rendering redacts credentials, query values, and fragments.

The UI is indeterminate while a product run owns the lifecycle and sends cancellation through `CancelAutoTune`; it stays open until the terminal result returns. It reports the exact product status, planner disposition, catalog status, candidate outcomes, and factual `state_restored` value. `STATE_RESTORE_FAILED` is critical. `COMPLETED_SELECTED` means only that a strategy was verified during the experiment and the original state was restored: no candidate is activated, saved, learned, or added to the legacy profile catalog.
