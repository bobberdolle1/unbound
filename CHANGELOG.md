...
- **Отказ от всех не-desktop платформ**: Удалены сырые и экспериментальные модули (Android VpnService/TUN, Magisk module, Steam Deck plugin, Web Extension, OpenWrt, LG WebOS, tvOS).
...
- **Перезапуск версионирования**: Сброс версии до **`0.1.0-refresh`**. Отказ от сырых релизов до достижения полной стабильности и отполированности десктопного ядра.
- **Реструктуризация Git**: Старое мультиплатформенное наследие версии v2.5.0 заархивировано на ветке **`legacy/v2.x`**.
- **Очистка инфраструктуры**: Удален рабочий процесс публикации GitHub Pages (`pages.yml`), очищены скрипты сборки и конфигурации CI под 3 целевые системы.
