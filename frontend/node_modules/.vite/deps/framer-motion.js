...
function applyGeneratorOptions({ type, ...options }) {
  ...
  if (isGenerator(type) && supportsLinearEasing()) {
    ...
    (_a = options.duration) != null ? _a : options.duration = 300;
    (_b = options.ease) != null ? _b : options.ease = "easeOut";
  }
  return options;
}
...
var NativeAnimation = class extends WithPromise {
  ...
  get startTime() {
    var _a;
    return (_a = this.manualStartTime) != null ? _a : Number(this.animation.startTime);
  }
...
function getValueTransition(transition, key) {
  var _a, _b;
  const valueTransition = (_b = (_a = transition == null ? void 0 : transition[key]) != null ? _a : transition == null ? void 0 : transition["default"]) != null ? _b : transition;
...
function animateTarget(visualElement, targetAndTransition, { delay: delay2 = 0, transitionOverride, type } = {}) {
  ...
  for (const key in target) {
    const value = visualElement.getValue(key, (_a = visualElement.latestValues[key]) != null ? _a : null);
...
function createEffect(addValue) {
  ...
  return (subject, values) => {
    var _a;
    const state = (_a = stateCache.get(subject)) != null ? _a : new MotionValueState();
  ...
  if (transformProps.has(key)) {
    ...
    if (!state.get("transformOrigin")) {
      state.set("transformOrigin", new MotionValue(""), () => {
        var _a, _b, _c;
        const originX = (_a = state.latest.originX) != null ? _a : "50%";
        const originY = (_b = state.latest.originY) != null ? _b : "50%";
        const originZ = (_c = state.latest.originZ) != null ? _c : 0;
  ...
  if (key === "pathOffset") {
    ...
    if (!state.get("stroke-dasharray")) {
      state.set("stroke-dasharray", new MotionValue("1 1"), () => {
        ...
        element.setAttribute("stroke-dasharray", `${pathLength} ${pathSpacing != null ? pathSpacing : 1 - Number(pathLength)}`);
      });
    }
...
function attachFollow(value, source, options = {}) {
  ...
  const scheduleAnimation = () => {
    var _a;
    startAnimation();
    value.animation = activeAnimation != null ? activeAnimation : void 0;
    ...
    transition.ready.then(() => {
      ...
      for (const animation of generatedViewAnimations) {
        ...
        if (!targetDefinition) {
          ...
          effect.updateTiming({
            delay: secondsToMilliseconds((_a = animationTransition.delay) != null ? _a : 0),
...
function transformBox(box, transform2, sourceBox) {
  const resolveBox = sourceBox != null ? sourceBox : box;
...
function buildSVGAttrs(state, {
  ...
  if (style2.transform || attrs.transformOrigin) {
    style2.transformOrigin = (_a = attrs.transformOrigin) != null ? _a : "50% 50%";
    delete attrs.transformOrigin;
  }
  if (style2.transform) {
    style2.transformBox = (_b = styleProp == null ? void 0 : styleProp.transformBox) != null ? _b : "fill-box";
...
function snapshotFromTarget(projection) {
  ...
  return {
    animationId: (_b = (_a = projection.root) == null ? void 0 : _a.animationId) != null ? _b : 0,
    measuredBox,
    layoutBox,
...
function getOrCreateRecord(element, parentProjection, projectionOptions) {
  ...
  const visualElement = existing != null ? existing : new HTMLVisualElement({
    props: {},
...
var createDomVisualElement = (Component3, options) => {
  var _a;
  const isSVG = (_a = options.isSVG) != null ? _a : isSVGComponent(Component3);
...
function getTimeline({ source, container, ...options }) {
  ...
  const targetKey = (_a = options.target) != null ? _a : "self";
  let targetCache = containerCache.get(targetKey);
  if (!targetCache) {
    targetCache = {};
    containerCache.set(targetKey, targetCache);
  }
  const axisKey = axis + ((_b = options.offset) != null ? _b : []).join(",");
...
function calcNextTime(current2, next2, prev, labels) {
  ...
  if (typeof next2 === "number") {
    ...
    return (_a = labels.get(next2)) != null ? _a : current2;
  }
}
...
function createAnimationsFromSequence(sequence, { defaultTransition = {}, ...sequenceTransition } = {}, scope, generators) {
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    const resolveValueSequence = (valueKeyframes, valueTransition, valueSequence, elementIndex = 0, numSubjects = 0) => {
      ...
      duration != null ? duration : duration = defaultDuration;
...
function useAnimate() {
  ...
  const reduceMotion = (_a = useReducedMotionConfig()) != null ? _a : void 0;
